...
- `LocalVoiceInputMac`: macOS app implementation using AppKit, Accessibility, AVFoundation, pasteboard, CGEvent, and URLSession WebSocket.
...
6. `FunASRClient` or `MockASRClient` emits `ASREvent` values into `TranscriptBuffer`.
...
- Active ASR callbacks are guarded by both session id and ASR client identity.
