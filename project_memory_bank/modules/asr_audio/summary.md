...
The ASR/audio path is local-first and supports mock validation plus real FunASR WebSocket 2-pass transcription.
...
- Qwen3-ASR MLX 0.6B cumulative recompute is promising on current local smoke and `long_120_001` probes, but any app path still requires process supervision, Swift-side client validation, cancellation/stale-result preservation, and resource limits.
...
- Qwen3-ASR MLX 0.6B passes the current local HTTP process-boundary gate on smoke, `long_120_001`, and the extended long-case subset (`long_200_001`, `long_400_001`, `long_code_switch_001`); the next integration step is Swift adapter/resource validation, not direct replacement of the existing FunASR app backend.
...
- Qwen3-ASR MLX cumulative recompute probes and HTTP service gates are wrapper-feasibility evidence only. They are not native realtime streaming and should not be wired into the app until Swift-side runtime validation preserves cancel, stale-session isolation, focus downgrade, clipboard fallback, and no partial insertion.
- The validated Qwen3 cumulative service boundary proves wrapper state-machine and local process-boundary feasibility, but it is not yet a durable app backend because it lacks process supervision, memory/CPU ceilings, and Swift-side runtime validation.
